...
const { SessionProvider, CapabilitiesProvider, WeighProvider, CheckProvider, fmt } = require('./providers');
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.window.registerTreeDataProvider('clawprint.session', session),
  ...
  async function refresh() {
    ...
    try {
      ...
      session.update(snap, root);
      capabilities.update(snap, root);
      weigh.update(snap, root);
      check.update(snap, root);

      if (snap.weigh) {
        // Prefer the full per-session total (global + project); fall back to the
        // project-only always tier if the CLI is too old to emit report.session.
        const tokens = snap.weigh.session?.totalTokens ?? snap.weigh.always.tokens;
        ...
        status.tooltip = `Clawprint: ~${fmt(tokens)} tokens loaded per session`
          + (snap.weigh.session ? ` (global ~${fmt(snap.weigh.session.globalAlwaysTokens)} + project ~${fmt(snap.weigh.session.projectAlwaysTokens)})` : '')
