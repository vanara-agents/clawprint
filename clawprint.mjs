...
export const FINDING_KINDS = ['tools', 'commands', 'installs', 'network', 'env', 'paths', 'opaque', 'symlinks'];
...
export function discoverItems(root, { configDir = '.claude' } = {}) {
  ...
  // Symlink escape: a file whose REAL location is outside the scan root means
  // this item's content can change without any diff in this repo — the link
  // itself must be visible at review time, not discovered at check time.
  let realRoot;
  ...
  for (const item of items) {
    ...
    for (const f of item.files) {
      ...
      if (real !== realRoot && !real.startsWith(realRoot + sep)) {
        external.push({ path: f.path, target: toPosix(real) });
      }
    }
    if (external.length) item.external = external;
  }
...
export function buildReport(items) {
  ...
  const reportItems = items.map((item) => {
    ...
    // Externally-sourced files (symlink escape) are findings in their own
    // right: the target can change with zero diff in this repo.
    for (const ext of item.external ?? []) {
      findings.push({ kind: 'symlinks', value: ext.target, file: ext.path, line: 1 });
    }
...
export const scanDir = (root, opts) => buildReport(discoverItems(root, opts));
...
export function renderMarkdown(report) {
  ...
  const lines = [
    ...
    '',
    '## Summary',
    '| Items | Commands | Installs | Network hosts | Env vars | Outside writes | Opaque blocks | External sources |',
    '|---|---|---|---|---|---|---|---|',
    `| ${s.items} | ${s.commands} | ${s.installs} | ${s.network} | ${s.env} | ${s.paths} | ${s.opaque} | ${s.symlinks} |`,
    '',
  ];
 ...
 *   tools.deny     [globs] — any granted tool matching one is a violation
 *   installs: false        — any runtime install is a violation
 *   installs.allow [globs] — any install NOT matching one is a violation
 *   paths: false           — any write outside the project is a violation
 *   opaque: false          — any opaque block is a violation
 *   symlinks: false        — any externally-sourced file is a violation
...
export function evaluatePolicy(report, policy) {
  ...
  const allowInstalls = policy.installs?.allow ? policy.installs.allow.map(policyGlob) : null;
  const denyNet = denyOf('network');
  const denyEnv = denyOf('env');
  const denyCmd = denyOf('commands');
  const denyTools = denyOf('tools');
  ...
  for (const item of report.items) {
    for (const f of item.findings) {
      ...
      } else if (f.kind === 'tools' && denyTools.some((re) => re.test(f.value))) {
        violations.push({ itemId: item.id, kind: f.kind, value: f.value, rule: 'tools.deny' });
      } else if (f.kind === 'installs') {
        if (policy.installs === false) {
          violations.push({ itemId: item.id, kind: f.kind, value: f.value, rule: 'installs: false' });
        } else if (allowInstalls && !allowInstalls.some((re) => re.test(f.value))) {
          violations.push({ itemId: item.id, kind: f.kind, value: f.value, rule: 'installs.allow (package not on allowlist)' });
        }
      ...
      } else if (f.kind === 'symlinks' && policy.symlinks === false) {
        violations.push({ itemId: item.id, kind: f.kind, value: f.value, rule: 'symlinks: false' });
      }
    }
  }
...
const hostOfUrl = (url) => {
  const m = /^(?:https?|wss?|ftp):\/\/([^\s/'"<>)\]]+)/i.exec(String(url ?? ''));
...
/**
 * Decide a PreToolUse hook event against manifest(s) + policy.
 * Covers: Bash (commands + hosts), WebFetch (host — the purest exfil vector),
 * and Write/Edit/MultiEdit/NotebookEdit (writes OUTSIDE the project root).
 * `reports` may be one report or an array (project + global manifests union).
 * Returns { verdict: 'allow'|'warn'|'block', reasons: [] } — pure for tests.
 */
export function guardDecision(hookEvent, reports, policy, { enforce = false, projectDir = null } = {}) {
  ...
  const caps = { commands: new Set(), network: new Set(), paths: new Set() };
  for (const r of reportList) {
    const c = manifestCapabilities(r);
    for (const v of c.commands) caps.commands.add(v);
    for (const v of c.network) caps.network.add(v);
    for (const item of r.items ?? []) {
      for (const f of item.findings ?? []) if (f.kind === 'paths') caps.paths.add(f.value);
    }
  }
  const policyCheck = (findings) => {
    if (!policy) return;
  ...
  if (tool === 'Bash') {
    const { commands, hosts } = extractFromCommand(input.command ?? '');
    if (reportList.length > 0) {
      // Commands are only checked when the manifest declares any — a config
      // that never mentions commands makes "unknown command" pure noise.
      // Hosts are always checked: they're the exfiltration vector.
      if (caps.commands.size > 0) {
        for (const c of commands) if (!caps.commands.has(c)) reasons.push(`command not in manifest: ${c}`);
      }
      for (const h of hosts) if (!caps.network.has(h)) reasons.push(`host not in manifest: ${h}`);
    }
    policyCheck([
    ...
    if (host) {
      if (reportList.length > 0 && !caps.network.has(host)) reasons.push(`host not in manifest: ${host} (WebFetch)`);
      policyCheck([{ kind: 'network', value: host }]);
    }
  } else if (WRITE_TOOLS.has(tool)) {
    const target = input.file_path ?? input.notebook_path ?? '';
    if (projectDir && target && isAbsolute(target)) {
      const root = resolve(projectDir);
      const abs = resolve(target);
      if (abs !== root && !abs.startsWith(root + sep)) {
        // Outside the project: fine only if the manifest already declares an
        // outside-write to that location (prefix match on declared targets).
        const declared = [...caps.paths].some((p) => abs.startsWith(resolve(p.replace(/^~($|[\\/])/, `${homedir()}$1`))));
        if (!declared) reasons.push(`write outside project: ${target}`);
        policyCheck([{ kind: 'paths', value: target }]);
      }
    }
  }
...
export function renderFleet(fleet) {
  ...
  for (const kind of ['network', 'env', 'installs', 'paths', 'opaque', 'symlinks']) {
...
export function buildBadge(report) {
  ...
  // Orange when a human can't eyeball everything: opaque blobs, or content
  // sourced from outside the repo via symlink.
  const color = s.opaque > 0 || s.symlinks > 0 ? 'orange' : 'brightgreen';
...
Usage:
  ...
  npx clawprint guard            PreToolUse hook: check live Bash/WebFetch/writes against the manifest
  npx clawprint policy           dry-run the policy against the current config (no manifest needed)
...
Flags:
  ...
  --global                       weigh: add ~/.claude totals · scan/check/policy: operate ON ~/.claude
                                 (a globally installed skill grants capabilities in EVERY project —
                                 give the global config a manifest of its own)
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === 'policy') opts.mode = 'policy';
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (opts.global && !['weigh', 'scan', 'check', 'policy'].includes(opts.mode)) {
    process.stderr.write('clawprint: --global applies to weigh, scan, check and policy modes\n');
    return 2;
  }

  // --global in scan/check/policy: operate on the machine-wide ~/.claude —
  // the config that rides along in EVERY project. A skill installed there
  // grants capabilities everywhere yet appears in no repo's manifest; this
  // gives it a manifest of its own (committed nowhere, living in ~/.claude —
  // check compares against the last scan).
  let scanOpts;
  if (opts.global && opts.mode !== 'weigh') {
    opts.dir = globalClaudeDir();
    scanOpts = { configDir: '.' };
    if (!existsSync(opts.dir)) {
      process.stderr.write(`clawprint: no global config at ${opts.dir}\n`);
      return 2;
    }
  }
  ...
  if (opts.mode === 'guard') {
    ...
    // Union the project manifest with the global one (~/.claude): globally
    // installed skills legitimately grant their capabilities in every repo.
    const reports = [];
    for (const dir of [opts.dir, globalClaudeDir()]) {
      const manifestPath = join(dir, MANIFEST_JSON);
      if (!existsSync(manifestPath)) continue;
      try { reports.push(JSON.parse(readFileSync(manifestPath, 'utf8'))); } catch { /* fail-open */ }
    }
    let policy = null;
    try { policy = loadPolicy(opts.dir); } catch (err) { process.stderr.write(`clawprint guard: ${err.message} — ignoring policy.\n`); }
    if (reports.length === 0 && !policy) {
      process.stderr.write(`clawprint guard: no ${MANIFEST_JSON} or ${POLICY_FILE} in ${opts.dir} — allowing (run npx clawprint first).\n`);
      return 0;
    }
    const { verdict, reasons } = guardDecision(event, reports, policy, { enforce: opts.enforce, projectDir: opts.dir });
  ...
  const report = scanDir(opts.dir, scanOpts);

  if (opts.mode === 'policy') {
    // Dry-run: what would the policy flag RIGHT NOW? No manifest required —
    // this is how you preview a policy before it starts failing CI.
    let policy;
    try { policy = loadPolicy(opts.dir); } catch (err) { process.stderr.write(`clawprint policy: ${err.message}\n`); return 1; }
    if (!policy) {
      process.stdout.write(`clawprint policy: no ${POLICY_FILE} in ${opts.dir}.\n`
        + 'Create one — e.g. {"network":{"allow":["api.github.com"]},"env":{"deny":["AWS_*"]}}\n');
      return 0;
    }
    const violations = evaluatePolicy(report, policy);
    if (opts.json) {
      process.stdout.write(`${JSON.stringify({ version: VERSION, violations }, null, 2)}\n`);
      return violations.length > 0 ? 1 : 0;
    }
    if (violations.length === 0) {
      process.stdout.write(`clawprint policy: clean — nothing in the current config violates ${POLICY_FILE}.\n`);
      return 0;
    }
    for (const line of renderPolicyViolations(violations)) process.stdout.write(`${line}\n`);
    process.stdout.write(`\nclawprint policy: ${violations.length} violation(s).\n`);
    return 1;
  }
  ...
  if (opts.mode === 'check') {
    ...
    if (opts.json) {
      // Machine-readable diff for bots and dashboards: change lines verbatim
      // (the same strings humans see — one format to parse), structured
      // policy violations, and the two gate booleans.
      const capabilityFail = breaking;
      const policyFail = policyViolations.length > 0;
      process.stdout.write(`${JSON.stringify({
        version: VERSION, changes: lines, policyViolations, capabilityFail, policyFail, ok: !capabilityFail && !policyFail,
      }, null, 2)}\n`);
      return capabilityFail || policyFail ? 1 : 0;
    }
